...
export async function downloadPriceEditExcel(products: Product[], suppliers: string[], region: string, sphere: string) {
    ...
    for (const p of products) {
        ...
        ws.addRow([
            ...
            region || '',
            p.unit || '',
            p.prices?.supplier2?.[region] !== undefined ? p.prices.supplier2[region] : '',
            p.prices?.supplier3?.[region] !== undefined ? p.prices.supplier3[region] : '',
            p.prices?.supplier4?.[region] !== undefined ? p.prices.supplier4[region] : ''
        ]);
    }
    ...
    instructionRow.addRow(['1. Не изменяйте значения в колонке ID (Колонка A) и Регион (Колонка E)! Они нужны для обновления товара.']);
    instructionRow.addRow(['2. Редактируйте цены в 3-х колонках поставщиков ("Поставщик 1", "Поставщик 2", "Поставщик 3").']);
    instructionRow.addRow(['3. Если нужно удалить цену, поставьте символ "-" (минус). Оставление пустой ячейки проигнорирует обновление.']);
    instructionRow.addRow(['4. Цены будут записаны ИМЕННО ДЛЯ указанного региона в строке.']);
    ...
    saveAs(blob, `Цены_${safeRegionName}_${safeSphereName}_${new Date().toLocaleDateString('ru-RU').replace(/\./g, '_')}.xlsx`);
}

export async function importPriceEditExcel(file: File): Promise<{ updated: number; errors: string[] }> {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (!ws) {
                    throw new Error("Лист с именем 'Цены' не найден. Убедитесь, что вы загружаете правильный файл.");
                }
                ...
                for (let i = 2; i <= rowCount; i++) {
                    ...
                    const getPriceVal = (cell: ExcelJS.Cell) => {
                        ...
                        if (val === null || val === undefined || val === '') return 'IGNORE';
                    ...
                    if (p2 === 'DELETE') updateData[`prices.supplier2.${region}`] = deleteField();
                    else if (p2 !== 'IGNORE') updateData[`prices.supplier2.${region}`] = p2;

                    if (p3 === 'DELETE') updateData[`prices.supplier3.${region}`] = deleteField();
                    else if (p3 !== 'IGNORE') updateData[`prices.supplier3.${region}`] = p3;

                    if (p4 === 'DELETE') updateData[`prices.supplier4.${region}`] = deleteField();
                    else if (p4 !== 'IGNORE') updateData[`prices.supplier4.${region}`] = p4;
